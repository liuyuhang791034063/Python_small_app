...
class Spider(CrawlSpider):
    ...
    rules = (
        # Rule(LinkExtractor(allow=('https://wallhaven.cc/wallpaper/\d{1,6}$',)), callback='parse_item'),
        Rule(LinkExtractor(allow=('https://wallhaven.cc/w/\w{1,6}$',)), callback='parse_item'),
    )
    ...
    def parse_item(self, response):
        ...
        for tag in ban_tags:
            if tag in tags:
                return
        item['url'] = str(response.xpath('//*[@id="wallpaper"]/@src').extract_first(default=0))
